...
export const returnBorrowedBook = catchAsyncErrors(async (req, res, next) => {
  ...
  res.status(200).json({
    success: true,
    message: fine !== 0 ? `Book returned successfully. The total charges, including fine, are ₹${fine + book.price}` : `Book returned successfully. The total charges are ₹${book.price}`,
  });
});
